...
export default function PeopleDetails() {
  ...
  const saveUser = async () => {
    ...
    const firstName = fullName[0] || user.firstName;
    const lastName = fullName[1] || user.lastName;
    const updatedUser = { ...user, firstName: firstName, lastName: lastName, role: role, email: email };
  ...
  return (
    <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      <div className="d-flex">
        <b>Roles:</b>
        {!editing && (
          <div className="wd-role" onClick={() => setEditing(true)}>
            {user.role}
          </div>
        )}
        {user && editing && (
