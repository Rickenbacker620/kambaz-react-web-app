        ...
        <ListGroup className="wd-lessons rounded-0">
          {assignments.map((assignment: any) => (
            <ListGroup.Item className="wd-lesson p-3 ps-1 d-flex align-items-center">
              ...
              <TfiWrite
                onClick={() => navigate(`/Kambaz/Courses/${assignment.course}/Assignments/${assignment._id}`)}
                className="me-4 fs-3 text-success"
              />
              <div className="flex-grow-1">
                <h2 className="wd-assignment-link text-decoration-none text-reset fw-bold fs-4">{assignment.title}</h2>
                <span className="text-danger">Multiple Modules</span> | <b>Not available until</b>{" "}
                {formatDate(assignment.from)} |
                <br />
